    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j += 4) {
                ...
                if (product > maxRow) maxRow = product;
            }
        }
        maxProducts.add(maxRow);

        int maxCol = 0;
        for (int j = 0; j < 20; j++) {
            for (int i = 0; i < 20; i += 4) {
                int product = grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j];
                if (product > maxCol) maxCol = product;
            }
        }
        maxProducts.add(maxCol);

        int maxDi = 0;
        for (int i = 0; i < 17; i++) {
            for (int j = 0; j < 17; j++) {
                int product = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
                if (product > maxDi) maxDi = product;
            }
        }
        maxProducts.add(maxDi);

        int maxDi2 = 0;
        for (int i = 0; i < 17; i++) {
            for (int j = 3; j < 20; j++) {
                int product = grid[i][j] * grid[i + 1][j - 1] * grid[i + 2][j - 2] * grid[i + 3][j - 3];
                if (product > maxDi2) maxDi2 = product;
            }
        }
